...
#[tauri::command]
pub async fn set_mission_header(
    state: tauri::State<'_, AppState>,
    mission_header_json: String,
) -> Result<(), String> {
    let parsed: serde_json::Value =
        serde_json::from_str(&mission_header_json).map_err(|e| e.to_string())?;
    let mut config = state.config.lock().await;
    config.config_mut().root.game.game_properties.mission_header = parsed;
    Ok(())
}
...
pub async fn save_server_configuration(state: tauri::State<'_, AppState>) -> Result<(), String> {
    ...
    let path = install_dir.join("server.json");
    file_io
